...
python utils/show_loss_graph.py "$1"/loss/ 1
mkdir "$1"/results
...
do
  ...
  if [ $(($i % 4)) = 0 ]; then
    suffix=with_mining.pth
  else
    suffix=without_mining.pth
  fi
  mkdir $out
  python Test_SSD_seg_fast.py --indir "$rgb" --outdir $out --gpu $gpu --model "$1"/model/SSD_Seg_epoch_"$i"_"$suffix"

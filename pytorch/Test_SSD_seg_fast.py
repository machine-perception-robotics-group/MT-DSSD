...
MODEL_PATH = "/home/ryorsk/SSDsegmentation/models_finished/2018-12-30@10-42-00_PyTorch1/model/SSD_Seg_epoch_150_without_mining.pth"
    ...
    for i in range(0, len(final_detections)):
            ...
            for j in range(0, len(final_detections[i])):
                ...
                if False: # ID表示
                    ...
                    cv.putText(out_img, str(i) + ": " + str(('%.2f' % final_detections[i][j][4])), (q1, q2 - 8), font, 1, (0, 0, 0), 2, cv.LINE_AA)
                    cv.putText(out_img, str(i) + ": " + str(('%.2f' % final_detections[i][j][4])), (q1, q2 - 8), font, 1, (255, 255, 255), 1, cv.LINE_AA)
                if True: # クラス名表示
                    cv.rectangle(out_img, (q1, q2 - 30), (q1 + len(class_name)*25, q2), (color[0], color[1], color[2]), -1)
                    cv.putText(out_img, str(i) + " " + class_name + ": " + str(('%.2f' % final_detections[i][j][4])), (q1, q2 - 8), font, 0.7, (0, 0, 0), 2, cv.LINE_AA)
                    cv.putText(out_img, str(i) + " " + class_name + ": " + str(('%.2f' % final_detections[i][j][4])), (q1, q2 - 8), font, 0.7, (255, 255, 255), 1, cv.LINE_AA)
...
def detection(img, ssd_model, filename, min_sizes, max_sizes):
    ...
    for g in range(1, len(labels)+1):
        ...
        # contours, hierarchy = cv.findContours(thresh, 1, 2) # OLD

...
# for hard neg mining
def set_epoch_num(train_chain, _log_report='LogReport'):
    @training.make_extension()
    def _set_epoch_num(trainer):
        log_report = trainer.get_extension(_log_report)
        now_log = log_report.log[len(log_report.log)-1]
        now_epoch = now_log['epoch']+1
        train_chain.set_epoch_num(now_epoch)
...
def loss_function(Loc, Cls, Seg, gt_box_batch, df_box_batch, idx_batch, cls_batch, bat_s, mining, seg_label, today_dir_path):
    ...
    # lossをstdout
    if DEBUG: print('confidence loss   : ' + str(loss_cls.data))
    if DEBUG: print('localization loss : ' + str(loss_loc.data))
    if DEBUG: print('segmentation loss : ' + str(loss_seg.data))
    ...
    if DEBUG: print(loss_cls, loss_loc, loss_seg)
    if DEBUG: print(type(loss_cls), type(loss_loc), type(loss_seg))
...
def main():
    ...
    if not path.exists(today_dir_path):
        ...
        os.mkdir(path.join(today_dir_path, "model"))
    #loss_log_path = path.join(today_dir_path, "loss.txt")

    output_dir = path.join(today_dir_path, "model")
    ...
    trainer.extend(set_epoch_num(train_chain), trigger=(1, 'epoch'))
    ...
    trainer.extend(extensions.PrintReport(
        ['epoch', 'iteration', 'lr',
         'main/loss', 'main/loss/cls', 'main/loss/loc', 'main/loss/seg', 'main/mining']),
    ...
    trainer.extend(
        extensions.snapshot_object(ssd_model, 'SSD_Seg_epoch_{.updater.epoch}_with_mining.model' if train_chain.mining else 'SSD_Seg_epoch_{.updater.epoch}_without_mining.model'),
        trigger=(1, 'epoch'))

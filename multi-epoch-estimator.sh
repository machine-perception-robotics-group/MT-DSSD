...
do
  ...
  mkdir $out
  python $2 --indir "$rgb" --outdir $out --gpu $gpu --model "$1"/model/SSD_Seg_epoch_"$i"_"$suffix"
  python utils/detect_evaluation.py --image "$rgb" --teach "$teach" --result "$out"/detection_txt/ --epoch $i
  python utils/segment_evaluation.py --teach "$segteach" --result "$out"/segmentation_results_gray/ --epoch $i
done
